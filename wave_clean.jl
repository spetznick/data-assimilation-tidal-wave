...
function update_kalman(X_new, L_matrix, H, observations, mode)
...
function simulate_enkf(settings, mode)
    (x, t0) = initialize!(settings)
    ilocs = settings["ilocs"]
    names = settings["names"]
    names = names[mode["location_used"]]
    t = settings["t"]

    A, B, full_state_data, X = construct_system(x, settings, mode)
    ...
    for i = 1:nt
        ...
        # Kalman update
        if mode["use_Kalman"]
            X, P = update_kalman(X, L_matrix, H, observed_data[:, i], mode)
        end
